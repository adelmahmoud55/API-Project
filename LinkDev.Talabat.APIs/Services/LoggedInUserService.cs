...
namespace LinkDev.Talabat.APIs.Services
{
    public class LoggedInUserService : ILoggedInUserService
    {
        ...
        public LoggedInUserService(IHttpContextAccessor? httpContextAccessor) // this inteface contain the http context to get the user id from the token
        {
            ...
            UserId = _httpContextAccessor?.HttpContext?.User?.FindFirstValue(ClaimTypes.PrimarySid)!;   // enta motalp lma t generste token{eli hwa hdfo n authriz and auth }  , set  name identifier 3shan t2dr t3ml get ll user id
        }
    }
}
